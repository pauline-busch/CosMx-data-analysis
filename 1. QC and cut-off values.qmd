...
group_assignment <- fov_df %>%
  mutate(group = case_when(
    ...
    fov >= 38  & fov <= 87  ~ "Tx-naïve_1",
    fov >= 88  & fov <= 105 ~ "Tx-naïve_2",
...
total_count_plot <- 
  ...
  geom_histogram(bins = 100, fill="blue", color="#e9ecef", alpha=0.5) +
  ...
  labs(
    title = "Distribution of the total transcript counts per cell",
    x = "Transcript count",
    y = ""
  ) +
...
negative_fraction_plot <- 
  ...
  geom_histogram(binwidth = 0.01, fill = "purple", colour = "purple", alpha = 0.6) +
  ...
  labs(
    title = "Negative fraction per cell",
...
total_count_grouped <- 
  ...
  labs(
    title = "Total transcript counts per cell - group comparison",
    x = "",
...
gene_density_grouped <- 
  ...
  geom_boxplot(fill = "blue", alpha = 0.5) +
  labs(
    title = "Detected gene count per cell - group comparison",
    x = "",
...
complexity_plot <- 
  ggplot(colData(cos_spe), aes(x = detected_genes, y = total_counts, color = negative_fraction)) +
  geom_point(alpha = 0.5) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(title = "Complexity: Total counts vs. detected Genes") +
  theme_minimal()
...
saveRDS(cos_spe_filtered, "C:/CosMx/Datasets/6k/TMA2/rds files/cos_spe_filtered.rds")
saveRDS(cos_spe_final, "C:/CosMx/Datasets/6k/TMA2/rds files/cos_spe_final.rds")
