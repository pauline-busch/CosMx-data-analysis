...
cos_path <- "C:/CosMx/Datasets/6k/TMA1" # Adjust file path if necessary

cos_spe <- readCosmxSXE(dirName = cos_path,
                        countMatPattern = "TMA1_biomedx_MAR_6K_exprMat_file.csv",
                        metaDataPattern = "TMA1_biomedx_MAR_6K_metadata_file.csv",
...
group_assignment <- fov_df %>%
  mutate(group = case_when(
    fov >= 1 & fov <= 11 ~ "Remission",
    fov >= 12 & fov <= 35 ~ "anti-TNF-IR",
    fov >= 36 & fov <= 69 ~ "Tx-naive",
    fov >= 70 & fov <= 89 ~ "csDMARD-IR"
  ))
...
slice_assignment <- fov_df %>%
  mutate(slice = case_when(
    fov >= 1 & fov <= 11 ~ "slice_1_TL",
    fov >= 12 & fov <= 35 ~ "slice_2_TR",
    fov >= 36 & fov <= 69 ~ "slice_3_BL",
    fov >= 70 & fov <= 89 ~ "slice_4_BR"
  ))
